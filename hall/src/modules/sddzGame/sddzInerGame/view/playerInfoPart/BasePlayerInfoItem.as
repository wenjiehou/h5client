...
//			loadGangEff();
		...
		protected function onClickSitdown(e:Event):void
		{
			...
			else if(model.sddzMsgs.ACK_RoomInfo.RoundCount > 0)
			{
			...
			var str:String = "本游戏工具为收费软件，按游戏"+typeStr+"扣除房卡。坐下不扣钱，开始游戏后才会扣钱，开始游戏后不予退还。是否花费" + AppConfig.costRule[UserData.roomtype][rule[1]]/3 +"房卡加入牌局？";
...
//			recordTf.text = obj.Diamond + "";
			...
			tingpaiImg.visible = false;//model.isTingVec[pos];
...
//			mingEffAni.stop();
		...
		/**游戏开始之后，显示地主和农民*/
		public function showDizhuNM(pos:int):void
		{
			var imgae:Image = _skin["shenfenImg"];
		...
		{
			_skin["shenfenImg"].visible = false;
		}
		
		/**更新玩家加倍情况*/
		public function updateJiabei(pos:int):void
		{
			if(model.jiabeiVec[pos] == true)
			{
				_skin["jiabeiImg"].visible = true;
			}
			else
			{
				_skin["jiabeiImg"].visible = false;
