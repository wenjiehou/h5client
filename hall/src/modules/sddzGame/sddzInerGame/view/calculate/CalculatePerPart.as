package modules.sddzGame.sddzInerGame.view.calculate
{
	...
	import modules.sddzGame.sddzInerGame.view.calculate.item.CalculatePerItem;
	...
	public class CalculatePerPart
	{
		...
		protected var _items:Vector.<CalculatePerItem> = new Vector.<CalculatePerItem>(3);
		...
		public function updateBtns():void
		{
			...
			if(model.isSelfSitDown)
			{
				if(model.sddzMsgs.ACKBC_Sddz_Total.Finished == undefined)
				{
					model.sddzMsgs.ACKBC_Sddz_Total.Finished = false;
				}
				if(model.sddzMsgs.ACKBC_Sddz_Total.Finished==true)
				{
					_skin.confirmBtn.visible = true;
					...
					if(AppConfig.autoPlay)
					{
						var jiesuanObj:Object = model.sddzMsgs.ACKBC_Sddz_Total;
		...
		public function show():void
		{
			...
			var calData:Object = model.sddzMsgs.ACKBC_Sddz_Total;
