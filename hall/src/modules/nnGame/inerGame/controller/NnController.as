package modules.nnGame.inerGame.controller
{
	...
	import modules.nnGame.inerGame.view.playerInfoPart.GiftEff;
	...
	/**四人斗地主*/
	public class NnController extends AbstractLayer
	{
		public function NnController()
		{
			super();
		...
		public function ACK_RoomInfo(info:Object):void
		{
			...
			{
				view.nnPage.timeTip.showReadyLeft(info.leftTime);
				model.isReConn = true;
			}
		...
		public function ACKBC_Sitdown(info:Object):void 
		{
			if (_actived) //判断当前模块是否打开
			{
				...
				if(info.uid == UserData.selfuid)
				{
					...
					{
						view.playerInfoPage.updatePlayerInfos(model.playerInfoVec);
						view.nnPage.handlePart.updateAllCards(model.handleCards);
						
						for(var i:int=0;i<model.chairNum;i++)
						{
							var player:Object = model.playerByPos(i);
							if(player == null)
							{
								continue;
							}
							
							if(model.stage == 1)//叫分阶段
							{
								
								//更新玩家抢庄的状态
								if(player.haveJiao == true)
								{
									view.playerInfoPage.showQiangBei(i,player.jiaofen);
								}
							}
							else if(model.stage == 2)//下注阶段
							{
								if (i != model.zhuangPos)
								{
									if(player.haveXiazhu == true)
									{
										view.playerInfoPage.showXiazhu(i,model.xiazhuVec[i]);
									}
								}
							}
							else if(model.stage == 3)//亮牌阶段
							{
								if(model.mingpaiVec[i] == true)
								{
									view.nnPage.handlePart.showComp(i);
								}
...
//				}
		...
		public function ACKBC_Ready(info:Object):void
		{
			if(actived)
			{
				...
				view.playerInfoPage.showReady(info.pos);
				...
				if(info.pos == model.selfChairPos)
				{
					...
					view.playerInfoPage.hideQiangAndXia();
					//view.playerInfoPage.reset();
					trace("ready..........");
...
//				view.srddzPage.outputPart.reset();
				...
				for(i=0;i<info.Cards.length;i++)
				{
					...
					view.nnPage.handlePart.updateOnePosAllCards(i,model.handleCards[i]);
					if(handleCards.length == 4 && model.isReConn == false)
					{
						view.nnPage.handlePart.playQipaiEff(i,false);
					}
					if(handleCards.length == 5 && model.isReConn == false)
					{
						view.nnPage.handlePart.playQipaiEff(i,true);
					}
...
//				model.baodaoCards =  info.BaodaoCards == undefined? [] :info.BaodaoCards;
				...
				{
					if(model.stage == 1)//抢庄阶段
					{
						view.nnPage.timeTip.showZhuangLeft(info.BaodaoNum);
						if(info.HaveJiao[i] == true)
						{
							if(model.playerByPos(i))
							{
								model.playerByPos(i).jiaofen = info.JiaoFen[i];
								model.playerByPos(i).haveJiao = true;
							}
						
							view.playerInfoPage.showQiangBei(i,info.JiaoFen[i]);
						}
						else
						{
							if(model.isSelfSitDown && i == model.selfChairPos)
							{
								view.nnPage.handleBtns.showQiangBox();
								
							}
							if(model.playerByPos(i))
							{
								model.playerByPos(i).haveJiao = false;
							}
						}
					}
					else
					{
						model.zhuangPos = info. DizhuIndex;
						model.xiazhuVec[i] = info.XiaZhu[i];
					
						
						if(model.stage == 2)
						{
							view.nnPage.timeTip.showXiazhuLeft(info.BaodaoNum);
							if (i != model.zhuangPos)
							{
								if(info.HaveJiabei[i] == true)
								{
									...
									if(model.playerByPos(i))
									{
										model.playerByPos(i).haveXiazhu = true;
									}
								}
								else
								{
									if(model.isSelfSitDown && i == model.selfChairPos)
									{
										view.nnPage.handleBtns.showFenBox();
									
									}
									
									if(model.playerByPos(i))
									{
										model.playerByPos(i).haveXiazhu = false;
									}
								}
							}
						}
						...
						{
							view.nnPage.timeTip.showLiangLeft(info.BaodaoNum);
							...
							if(info.LastOpt[i] == 0)//没有亮牌
							{
								if(model.isSelfSitDown)
								{
									view.nnPage.handleBtns.showCtlBtns();
									
								}
							}
							else if(info.LastOpt[i] == 1)//亮牌了
							{
		...
		public function ACKBC_Sddz_JiaoDizhu(info:Object):void
		{
			if(actived)
			{
				...
				view.nnPage.timeTip.showZhuangLeft();
		...
		public function ACKBC_Sddz_Jiaofen(info:Object):void
		{
			if(actived)
			{
				...
				model.playerByPos(pos).jiaofen = info.Fen;
				model.playerByPos(pos).haveJiao = true;
				
				view.playerInfoPage.showQiangBei(pos,info.Fen);
				...
				SoundManager.playSound("res/sounds/pinshi/"+SoundPlay.instance.getSexRef(model.playerByPos(pos).sex)+soundStr);
		...
		public function ACKBC_Sddz_Dizhu(info:Object):void
		{
			if(actived)//这个时候要给农民显示加倍按钮 地主选择加倍和明牌按钮 赢藏所有叫分按钮
			{
				...
				var pos:int = model.getPlayerPosByUid(info.Uid);
				model.zhuangPos = pos;//确认地主的位置
				model.difen = info.Fen;
				
				model.stage = 2;
				
				var betweenArr:Array = [];
				
				for(var i:int=0;i<model.chairNum;i++)
				{
					if(model.playerByPos(i) != null && model.playerByPos(i).jiaofen == model.difen)
					{
						betweenArr.push(i);
					}
				}
				
				if(betweenArr.length == 0)//这种抢矿下大家都没有抢
				{
					for(var i:int=0;i<model.chairNum;i++)
					{
						if(model.playerByPos(i) != null )
				...
				else
				{
					...
					view.playerInfoPage.playQZAni(betweenArr,pos,this,onQZAniEnd);
				}
				
			
				
				
//				view.playerInfoPage.showZhuang();//显示地主农民
//				view.nnPage.handleBtns.reset();
//				if(model.zhuangPos != model.selfChairPos)
//				{
//					view.nnPage.handleBtns.showFenBox();
//				}
//				
//				view.nnPage.timeTip.showXiazhuLeft();
				
			}
		}
		
		//frameNum 经历了多少侦
		protected function onQZAniEnd(frameNum:int):void
		{
			...
			view.nnPage.timeTip.showXiazhuLeft(Math.round(10-frameNum/30));
		}
		...
		public function ACKBC_Sddz_Mingpai(info:Object):void
		{
			if(actived)
			{
				...
				{
					view.nnPage.handlePart.showComp(pos);
				}
			}
		}
...
//					view.srddzPage.actTipPart.showActTip(pos,BaseActTipItem.ACTTIP_BUJIABEI);
		...
		/**通知玩家下注*/
		public function ACKBC_Nn_Xiazhu(info:Object):void
		{
			if(actived)
			{
//				int32  Uid = 1;		//UserID
//				int32  Fen = 2;      //玩家的下注
				...
				trace("ACKBC_Nn_Xiazhu",info,"ACKBC_Nn_Xiazhu");
				model.srddzMsgs.ACKBC_Nn_Xiazhu = info;
				var pos:int = model.getPlayerPosByUid(info.Uid);
				
				model.playerByPos(pos).haveXiazhu = true;
				model.xiazhuVec[pos] = info.Fen;
				
				view.playerInfoPage.showXiazhu(pos,info.Fen);
		...
		public function ACKBC_CurPlayer(info:Object):void
		{
			if(actived)
			{
				...
				view.nnPage.handleBtns.showCuo();
				view.nnPage.timeTip.showLiangLeft();
...
//			view.srddzPage.dipaiPart.showCards(model.dipaiCards );
			...
			for(i=0;i<info.Cards.length;i++)
			{
				...
				model.updateHandleCards(info.Cards[i].Seat ,handleCards);
				view.nnPage.handlePart.updateOnePosAllCards(info.Cards[i].Seat ,model.handleCards[info.Cards[i].Seat]);
				
				//
				
				var paixing:int = PaixingLogic.getPaixing(model.handleCards[info.Cards[i].Seat]);
				
				view.nnPage.handlePart.showPaixing(info.Cards[i].Seat,paixing);
				
				Laya.timer.once(200*i,this,playEndPaixing,["res/sounds/pinshi/"+SoundPlay.instance.getSexRef(model.playerByPos(info.Cards[i].Seat).sex)+"niu_"+paixing+".mp3"]);
			
				
			}
		}
		
		protected function playEndPaixing(sound:String):void
		{
		...
		{
			//更新玩家的分数  后面需要加动画的
			var info:Object = model.srddzMsgs.ACKBC_Sddz_Total;
			
			var list:Array =  info.List;//s
			var player:Object;
			for(var i:int=0;i<list.length;i++)
			{
				if(model.playerByPos(i) != null)//这个位置有玩家，更新下数据
				{
					...
					view.playerInfoPage.updateOnePlayerGolds(player);
				}
			}
			
			//播放输给庄家的金币动画
			var hasShuZhuang:Boolean = false;
			for(var i:int=0;i<list.length;i++)
			{
				if(model.playerByPos(i) != null)//这个位置有玩家，更新下数据
				{
					player = 	model.playerByPos(i);
					if(i != model.zhuangPos)//不是庄家
					{
						if(player.score < player.preScore)//输了钱
						{
							view.playerInfoPage.playDropJinBiTo(i,model.zhuangPos);
							hasShuZhuang = true;
						}
					}
				}
			...
			if(hasShuZhuang == false)//没有人输给庄家，直接播放庄家煮给别人
			{
				playZSJinbiAni(model.zhuangPos);
			}
			else{
				Laya.timer.once(1500,this,playZSJinbiAni,[model.zhuangPos]);
			}
...
//			view.calculatePage.show();
			...
			{
				//Laya.timer.once(4000, this, contineGameQueue);
				view.playerInfoPage.showGameEndBtn();
				view.nnPage.timeTip.reset();
			...
			else
			{
				...
				view.nnPage.timeTip.showReadyLeft(parseInt(info.Attached));//model.srddzMsgs.ACKBC_Sddz_Card_Init.BaodaoNum
			}
			
		}
		
		/**播放庄家输给别人的动画*/
		protected function playZSJinbiAni(zhuangPos:int):void
		{
			var info:Object = model.srddzMsgs.ACKBC_Sddz_Total;
			
			var list:Array =  info.List;
			var player:Object;
			for(var i:int=0;i<list.length;i++)
			{
				if(model.playerByPos(i) != null)//这个位置有玩家，更新下数据
				{
					...
					if(i != zhuangPos)//不是庄家
					{
						if(player.score > player.preScore)//输了钱
						{
							view.playerInfoPage.playDropJinBiFrom(i,zhuangPos);
						}
					}
				}
			}
...
//			view.calculatePage.reset();//影藏结算界面
		...
		public function sendMsgCommond(commondId:Object,needCantrol:Boolean=true):void
		{
			if(needCantrol)
			{
				view.nnPage.handlePart.canControl(model.selfChairPos,false);
			}
