package modules.nnGame.inerGame.view
{
	...
	import modules.nnGame.inerGame.view.playerInfoPart.UserInfoPage;
	...
	import ui.niuniuGame.GameTableUI;
	import ui.niuniuGame.PockerPageUI;
	
	public class NnView extends AbstractLayer
	{
		...
		protected var _nnPage:NnPage;
		public function get nnPage():NnPage
		{
			return _nnPage;
		}
...
//		}
		...
		protected function onAddUI():void
		{
			...
			if(_nnPage == null)
			{
				_nnPage = new NnPage(new PockerPageUI());
				_gui.pokerContainer.addChild(_nnPage.skin);
			}
...
//			}
			...
			_gui.popChat4.visible = false;
			_gui.popChat5.visible = false;
			_gui.popChat6.visible = false;
			_gui.popChat7.visible = false;
			_gui.popChat8.visible = false;
			_gui.popChat9.visible = false;
		...
		private function initPopChat(chairNum:int=10):void
		{
			...
			switch(chairNum)
			{
				...
				case 10:popChatArr.push(_gui.popChat0,_gui.popChat1,_gui.popChat2,_gui.popChat3,_gui.popChat4,_gui.popChat5,_gui.popChat6,_gui.popChat7,_gui.popChat8,_gui.popChat9);break;
		...
		private function showPopChat(uid:*,worlds:String):void
		{
			...
			{
				popChatArr[dir].x = 710 + (423 - popChatArr[dir].backimg.width)*0.6;
			}
			else if(popChatArr[dir] == _gui.popChat2)
			{
				popChatArr[dir].x = 678 + (423 - popChatArr[dir].backimg.width)*0.6;
			}
			else if(popChatArr[dir] == _gui.popChat3)
			{
				popChatArr[dir].x = 551 + (423 - popChatArr[dir].backimg.width)*0.6;
			}
			else if(popChatArr[dir] == _gui.popChat4)
			{
				popChatArr[dir].x = 420 + (423 - popChatArr[dir].backimg.width)*0.6;
			}
		...
		public function Voice():void
		{
			...
			Browser.window.wx.onVoicePlayEnd({
				success: function (res):void {
					...
					Laya.timer.clear(NnGameModule.instance.gameContext.view,recoverSoundVolume);
